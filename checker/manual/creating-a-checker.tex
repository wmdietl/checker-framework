...
(\chapterpageref{units-checker}), and a typestate checker
...
Write a \sunjavadocanno{java/lang/annotation/Target.html}{Target}
meta-annotation on the annotation definition to indicate where the annotation may be written.
All type qualifiers that users can use should have the value \<ElementType.TYPE\_USE>.
The checker-framework automatically considers any annotation that has this value as a type qualifier.
(An annotation that is written on an annotation
definition, such as \sunjavadocanno{java/lang/annotation/Target.html}{Target}, is called a \emph{meta-annotation}.)

% \noindent
% The \<@Target({ElementType.TYPE_USE})> meta-annotation
...
% declaration does not bear the \<@Target({ElementType.TYPE_USE})> meta-annotation (with minor
...
This qualifier is added to the IGJ checker manually in IGJAnnotatedTypeFactory.
...
polymorphic qualifiers), either by placing the annotations inside a subfolder called \<qual> located in the same directory as the checker, or by overriding
the
\refmethodterse{framework/type}{AnnotatedTypeFactory}{createSupportedTypeQualifiers}{--}
method. By default, the Polymorphic Qualifier \refqualclass{framework/qual}{PolyAll} is not utilized, but can be optionally added to a checker.

There's four recommended ways to write an override implementation of \refmethodterse{framework/type}{AnnotatedTypeFactory}{createSupportedTypeQualifiers}{--}

\begin{enumerate}
\item using reflective lookup to add all qualifiers, and add \code{@PolyAll}. Example:
...
\item using reflective lookup to add qualifiers, add \code{@PolyAll}, and add a manual
...
the \<qual> folder, or for qualifiers which do not or cannot have the
\<@Target(\{ElementType.TYPE\_USE\})> meta-annotation (eg some Bottoms).

\item using reflective lookup to add qualifiers and add a manual list of
qualifiers, but without \code{@PolyAll}. Example:
...
\item only using a manual list of qualifiers. Example:
...
Each value returned by \<createSupportedTypeQualifiers> is a class literal for a type qualifier
whose definition bears the \<@Target(\{ElementType.TYPE\_USE\})> meta-annotation.
...
\mbox{\code{org.checkerframework.framework.test.CheckerFrameworkTest}},
...
\subsection{Using an external debugger\label{debugging-options-external}}
...
% LocalWords:  TypeHierarchy GraphQualifierHierarchy ReadOnly Foo
...
%%  LocalWords:  MyChecker's subchecker plugins
